...
def generate_matrix():
    def f(X1, X2, X3):
        y = 0.6 + 8.0 * X1 + 8.8 * X2 + 9.2 * X3 + 4.7 * X1 * X1 + 0.1 * X2 * X2 + 1.1 * X3 * X3 + 6.3 * X1 * X2 + \
            0.4 * X1 * X3 + 1.3 * X2 * X3 + 2.9 * X1 * X2 * X3 + randrange(0, 10) - 5
        return y
...
while True:
    ...
    if fischer_test():
        ...
        print('Математична модель не є адекватною експериментальним даним.')
        x1[0] *= 1.7
        x1[1] *= 1.7
        x2[0] *= 1.7
        x2[1] *= 1.7
